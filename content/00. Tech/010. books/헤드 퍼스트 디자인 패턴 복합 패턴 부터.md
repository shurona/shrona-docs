...
|        | 프록시 패턴 (Proxy)            | 객체를 감싸서 그 객체로의 접근을 제어한다.                                                              |           |
...
|        | 템플릿 메소드 (Template Method) | 알고리즘의 개별 단계를 구현하는 방법을 서브 클래스에서 결정한다.                                                  | O         |
|        | 커맨드 패턴 (Command Pattern)  | 요청을 객체로 감싼다.  <br>(요청 자체를 객체로 캡슐화 해서 요청의 실행, 취소 등을 처리한다.)                             |           |
| **생성** | 팩토리 메소드 (Factory Method)  | 서브클래스에서 생성할 구상 클래스를 결정한다.  <br>객체의 생성로직을 서브 클래스에서 결정하게 한다.                            | O         |
|        | 싱글톤 (Singleton)           | 딱 한 개의 객체만 생성되도록 한다.                                                                  |           |
|        | 추상 팩토리 (Abstract Factory) | 클라이언트에서 구상 클래스를 지정하지 않으면서도 객체군을 생성할 수 있게 해준다.                                         | O         |
### 범주란
- 생성 패턴
	- 객체 인스턴스를 생성하는 패턴
